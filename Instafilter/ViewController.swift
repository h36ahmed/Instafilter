...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @IBAction func changeFilterDidTouch(_ sender: Any) {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        ...
        // updateFilter()  -- Easy Way
        
        useFilters() // -- Functional Programming Way
    }
    ...
    func useFilters() {
        let radius = 1.0
        let color = UIColor.green.withAlphaComponent(0.1)
        let blurAndOverlay = compose(filter: blur(radius: radius),
                                     with: overlay(color: color))
        let coreImage = CIImage(image: currentImage)
        let result1 = blurAndOverlay(coreImage!)
        let processedImage = UIImage(ciImage: result1)
